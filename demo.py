...
def demo(sess, net, image_name):
    ...
    for cls_ind, cls in enumerate(CLASSES[1:]):
        ...
        write_csv(image_name, dets, thresh=CONF_THRESH)

def write_csv(image_name, dets, thresh=0.5):
    """Write detected bounding boxes to csv file."""
    inds = np.where(dets[:, -1] >= thresh)[0]
    if len(inds) == 0:
        return
    ...
    with open(temp_file,'x') as tempfile:
        ...
        for i in inds:
            ...
            w = int(bbox[2] - bbox[0])
            h = int(bbox[3] - bbox[1])
            box_i = '{:d}_{:d}_{:d}_{:d}'.format(x,y,w,h)
            tempfile.write(box_i)
            if i==inds[-1]:
                tempfile.write('\n')
            else:
                tempfile.write(';')
